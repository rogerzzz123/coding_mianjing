...
def process_trade(orders, inventoty):
    ...
    for order in orders:
        symbol, action, quantity, price = order
        if quantity.startswith("$"):
            shares=int(quantity[1:])*100//price
        else:
            shares=int(quantity)

        current_inventory=inventory_dict.get(symbol,0)
        if action=="B":
            if current_inventory>shares:
                inventory_dict[symbol]-=shares
            else:
                shares_needed=shares-current_inventory
                shares_to_buy=100*(shares_needed+99)//100
                inventory_dict[symbol]=(shares_to_buy-shares_needed)
        elif action=="S":
            inventory_dict[symbol]+=shares
            if inventory_dict[symbol]>=100:
                inventory_dict[symbol]%=100
    return [[symbol, str(quantity)] for symbol, quantity in inventory_dict.items()]